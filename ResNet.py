...
def predict(net,test_iter,n=20,device=None):
    if isinstance(net, torch.nn.Module):
        net.eval()  # 设置为评估模式
        if not device:
            device = next(iter(net.parameters())).device
    ...
    X=X.cpu()
    d2l.show_images(X[0:n].reshape((n,28,28)),2,n//2,titles=titles[0:n])
    d2l.plt.show(wait=True)
...
net.load_state_dict(torch.load("mlp.params",weights_only=True))
train(net,train_iter,test_iter,num_epochs,lr,d2l.try_gpu())
# predict(net,test_iter,20,d2l.try_gpu())

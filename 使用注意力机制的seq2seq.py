...
for eng, fra in zip(engs, fras):
    translation, attention_weight_seq = predict_seq2seq(net, eng, src_vocab, tgt_vocab, num_steps, device, save_attention=True)
    print(f'{eng} => {translation}, bleu {bleu(translation.split(), fra.split(), k=2):.3f}')
    
attention_weights = torch.cat([step[0][0][0] for step in attention_weight_seq], 0).reshape((1, 1, -1, num_steps))
d2l.show_heatmaps(attention_weights[:, :, :, :len(engs[-1].split()) + 1].cpu(), 
                  xlabel='Key posistion', ylabel='Query position', titles=[''])
plt.show()

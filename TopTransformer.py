...
class DotProductAttention(nn.Module):
    def __init__(self, dropout, dimension, low_dimension, bias, **kwargs):
        super(DotProductAttention, self).__init__(**kwargs)
        self.dropout = nn.Dropout(dropout)
        self.trs = nn.Linear(dimension, low_dimension, bias=bias)
    
    def forward(self, queries, keys, values, valid_lens, top=None):
        queries_low = self.trs(queries)
        ...
        top = min(top,keys.shape[1])
        
        if valid_lens is not None:
            shape=scores_low.shape
            if valid_lens.dim()==1:
                valid_lens=torch.repeat_interleave(valid_lens,shape[1])
            else:
                valid_lens=valid_lens.reshape(-1)
            scores_low = d2l.sequence_mask(scores_low.reshape(-1,shape[-1]),valid_lens,value=-1e6)
            scores_low = scores_low.reshape(shape)
        
        _, topk_indices = torch.topk(scores_low, k=top, dim=2)
        
        bh, n, d = queries.size()
        indices_exp = topk_indices.unsqueeze(-1).expand(bh, n, top, d)  # [bh, n, top, d]
        
        m = keys.shape[1]
        
        q_high = queries.unsqueeze(-2)  # [bh, n, 1, d]
        k_high = torch.gather(keys.unsqueeze(1).expand(bh, n, m, d), dim=2, index=indices_exp)   # [bh, n, m, d] -> [bh, n, top, d]
        v_high = torch.gather(values.unsqueeze(1).expand(bh, n, m, d), dim=2, index=indices_exp)   # [bh, n, m, d] -> [bh, n, top, d]
        
        # print(bh, n, top, d, q_high.shape, k_high.shape)
        q_high = q_high.reshape(-1, 1, d)  # [bh*n, 1, d]
        k_high = k_high.reshape(-1, top, d)  # [bh*n, top, d]
        v_high = v_high.reshape(-1, top, d)  # [bh*n, top, d]
        
        score_high = torch.bmm(q_high, k_high.transpose(1,2)) / math.sqrt(q_high.shape[-1])
        score_high = score_high.reshape(bh, n, top)  # [bh, n, top]
        
        # 使用scatter进行批量修正
        # corrected_scores = scores_low.clone()
        # corrected_scores.scatter_(2, topk_indices, score_high)
        
        self.attention_weights = masked_softmax(score_high, valid_lens)
        return torch.bmm(self.dropout(self.attention_weights), v_high)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, key_size, query_size, value_size, num_hiddens, num_heads, dropout, bias=False, **kwargs):
        ...
        dimension = num_hiddens // num_heads
        low_dimension = max(dimension//B, 4)
        self.attention = DotProductAttention(dropout, dimension, low_dimension, bias)
    ...
    def forward(self, queries, keys, values, valid_lens):
        ...
        top = max(4, values.shape[1]//B)
        output = self.attention(queries, keys, values, valid_lens, top=None)
...
num_hiddens, num_layers, dropout, batch_size, num_steps = N1, 2, 0.1, 64, 100
lr, num_epochs, device = 0.005, 100, d2l.try_gpu()
ffn_num_input, ffn_num_hiddens, num_heads = N1, N1<<1, 2
key_size, query_size, value_size = N1, N1, N1
norm_shape = [N1]
